...
st.markdown("Ask me about monthly financials and I'll give you **board-ready answers**.")

query = st.text_input(
    "Ask a finance question (e.g., 'What was June 2025 revenue vs budget?')"
)
...
if query:
    ...
    if not intent:
        st.warning(
            "⚠️ I didn’t understand that. Try asking about revenue vs budget, gross margin, opex, or cash runway."
        )
        ...
        if intent == "revenue_vs_budget":
            ...
            st.success(
                ...
                f"Variance = {result['variance']:,.0f} "
                f"({result['variance_pct']}%)."
            )
        ...
        # ---------------- CASH RUNWAY ----------------
            ...
            st.success(
                ...
                f"📉 Avg Burn (last 3 months) = **${result['avg_burn']:,.0f}** "
            ...
            fig.update_yaxes(title="Cash (USD)")
            st.plotly_chart(fig, use_container_width=True)
